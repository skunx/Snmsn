...
class SnGtkWindow():
    ...
    def selection(self, widget, event, data=None):
        """Shows the selected contact item in data window label zone,
    ...
    def create_sn_tree(self):
        """Creates (after erasing old) and shows a treeview based on the Sn_Gui object content"""
        sn = self.sn
    ...
    def add_if_not_sn_account_tree(self, ip, acc):
        """Add the ip->acc to the tree if doesn't exists and return the iter

        @param ip: the ip of the account
    ...
    def add_if_not_sn_contact_tree(self, ip, acc, cont):
        """Add the ip->acc->cont to the tree if doesn't exists and return the iter
