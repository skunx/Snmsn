...
class Sn_Gtk_Window():
    ...
    def Update_Sn_Tree(self):
        ...
        for ip in sn.list_ips():
            self.AddIfNot_Sn_ip_Tree(ip)
            for acc in sn.ip(ip).list_accounts():
                self.AddIfNot_Sn_account_Tree(ip,acc)
                for cont in sn.ip(ip).account(acc).list_contacts():
                    self.AddIfNot_Sn_contact_Tree(ip,acc,cont)
        
    def show(self):
        self.window.show_all()
        
    def AddIfNot_Sn_ip_Tree(self,ip):
        """AddIfNot_Sn_ip_Tree(ip): add the ip to the tree if doesn't exists and return the iter"""
        tr=self.explorer_tree
    ...
    def AddIfNot_Sn_account_Tree(self,ip,acc):
        """AddIfNot_Sn_account_Tree(ip,acc): add the ip->acc to the tree if doesn't exists and return the iter"""
        ipiter=self.AddIfNot_Sn_ip_Tree(ip)
        tr=self.explorer_tree
        is_acc=False
    ...
    def AddIfNot_Sn_contact_Tree(self,ip,acc,cont):
        """AddIfNot_Sn_contact_Tree(ip,acc,cont): add the ip->acc->cont to the tree if doesn't exists and return the iter"""
        acciter=self.AddIfNot_Sn_account_Tree(ip,acc)
        tr=self.explorer_tree
        is_cont=False
...
if __name__ == "__main__":
    ...
    tvSn.Create_Sn_Tree()
    tvSn.AddIfNot_Sn_account_Tree("192.168.0.11","luf@hotmail.ci")
    ...
    tvSn.AddIfNot_Sn_account_Tree("192.168.0.10","luf@hotmail.ci")
    tvSn.Update_Sn_Tree()
    tvSn.show()
